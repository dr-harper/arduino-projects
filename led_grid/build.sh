#!/usr/bin/env bash
# Build script for LED grid firmware.
# Usage:
...
SKETCH_DIR="."
BIN="build/esp32.esp32.nologo_esp32c3_super_mini/led_grid.ino.bin"
DEFAULT_IP="${LED_GRID_IP:-tetris.local}"
DEVICE_PASS="${LED_GRID_PASS:-}"
...
if [[ "${1:-}" == "--upload" ]]; then
    if [[ -z "$DEVICE_PASS" ]]; then
        echo "Error: Set LED_GRID_PASS environment variable (your device password)" >&2
        exit 1
    fi
    IP="${2:-$DEFAULT_IP}"
